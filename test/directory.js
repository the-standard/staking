...
beforeEach(async () => {
  ...
  DirectoryContract = await ethers.getContractFactory('Directory');
...
describe.only('StakingDirectory', async () => {
  ...
  it('tests ownership', async () => {
    ...
    await expect(add).to.be.revertedWithCustomError(DirectoryContract, 'OwnableUnauthorizedAccount')
      .withArgs(user1.address);

    const del = Directory.connect(user1).del(addr1);
    await expect(del).to.be.revertedWithCustomError(DirectoryContract, 'OwnableUnauthorizedAccount')
      .withArgs(user1.address);
  });
});