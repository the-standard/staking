...
describe('Staking', async () => {
  it('opens the pool and sets all the variables', async () => {
    ...
    const Staking = await StakingContract.deploy("Staking", "STS", start, endTime, maturity, TST.address, SEuro.address, simpleInterestRate);
  ...
  it('activates the pool', async () => {
    const Staking = await StakingContract.deploy("Staking", "STS", 2000, 5000, 2600, TGateway.address, TST.address, SEuro.address, simpleInterestRate);
    expect(await Staking.active()).to.eq(false);
  ...
  it('disables the pool', async () => {
    const Staking = await StakingContract.deploy("Staking", "STS", 1000, 200000000000000, 5000, TGateway.address, TST.address, SEuro.address, simpleInterestRate);
  ...
  it('refuses to mint after a user already claimed a reward', async () => {
    ...
    const Staking = await StakingContract.deploy("Staking", "STS", start, 2000000000000, start, TGateway.address, TST.address, SEuro.address, simpleInterestRate);
    await Staking.activate();
  ...
  describe('SEURO balance', async () => {
    ...
    it('tests for the seuro remaining', async () => {
      let blockNum = await ethers.provider.getBlock();
      const start = blockNum.timestamp;
      const Staking = await StakingContract.deploy("Staking", "STS", start, start + 3600, 5000, TGateway.address, TST.address, SEuro.address, simpleInterestRate);
  ...
  describe('mint and burn rate calculations', async () => {
    ...
    it('tests the start, end, and validate stakes', async () => {
      ...
      const Staking = await StakingContract.deploy("Staking", "STS", then, then + 600, 100, TGateway.address, TST.address, SEuro.address, simpleInterestRate);
    ...
    it('tests the exchange rate', async () => {
      ...
      let Staking = await StakingContract.deploy("Staking", "STS", then, then + 600, then + 100, TGateway.address, TST.address, SEuro.address, simpleInterestRate);
      ...
      Staking = await StakingContract.deploy("Staking", "STS", then, then + 600, then + 5000, TGateway.address, TST.address, SEuro.address, simpleInterestRate);
      await Staking.activate();
    ...
    it('burns and withdraws seuro', async () => {
      ...
      const Staking = await StakingContract.deploy("Staking", "STS", then, then + 600, then + 5000, TGateway.address, TST.address, SEuro.address, simpleInterestRate);
